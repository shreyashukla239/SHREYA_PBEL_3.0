'use client';
import React, { useState, useEffect } from 'react';
import Link from 'next/link';
import { fetchAttendanceStreaks, StreakEntry } from '../lib/apiClient';
export default function StreaksPage(): React.JSX.Element {
  const [streaks, setStreaks] = useState<StreakEntry[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [errorMsg, setErrorMsg] = useState<string>('');
  const [searchTerm, setSearchTerm] = useState<string>('');
  const loadStreaks = async () => {
    setLoading(true);
    setErrorMsg('');
    try {
      const res = await fetchAttendanceStreaks();
      if (res.status === 'success') {
        setStreaks(res.streaks);
      } else {
        setErrorMsg('Invalid streaks response format.');
      }
    } catch (err: unknown) {
      setErrorMsg(
        err instanceof Error ? err.message : 'Failed to fetch streaks.'
      );
    } finally {
      setLoading(false);
    }
  };
  useEffect(() => {
    loadStreaks();
  }, []);
  const filteredStreaks = streaks.filter((entry) =>
    entry.display_name.toLowerCase().includes(searchTerm.toLowerCase())
  );
  const getBadgeStyle = (badge: string) => {
    if (badge.startsWith('Early Bird')) {
      return 'bg-emerald-50 text-emerald-700 border-emerald-100';
    }
    if (badge.startsWith('Streak Master')) {
      return 'bg-amber-50 text-amber-700 border-amber-100';
    }
    return 'bg-blue-50 text-blue-705 border-blue-100';
  };
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 sm:p-6 text-slate-900">
      <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 max-w-4xl w-full mx-auto shadow-sm flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Attendance Streaks &amp; Badges</h1>
            <p className="text-slate-500 text-xs mt-1">Track consecutive student check-in history records</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={loadStreaks}
              className="bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs py-2 px-3.5 rounded-xl border border-slate-200 transition-colors cursor-pointer"
            >
              🔄 Refresh
            </button>
          </div>
        </div>
        <div className="relative">
          <input
            type="text"
            placeholder="Search by student name..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-3 pr-10 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm font-semibold text-slate-800"
          />
          <div className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
            🔍
          </div>
        </div>
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            <svg className="animate-spin h-8 w-8 text-blue-600" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
            </svg>
            <span className="text-slate-500 font-medium text-xs">Evaluating biometric logs...</span>
          </div>
        ) : errorMsg ? (
          <div className="bg-rose-50 border border-rose-250 rounded-xl p-6 text-center flex flex-col items-center gap-3">
            <span className="font-bold text-rose-950 text-sm">Calculation Failure</span>
            <p className="text-rose-700 text-xs">{errorMsg}</p>
            <button
              type="button"
              onClick={loadStreaks}
              className="bg-rose-600 hover:bg-rose-700 text-white font-semibold text-xs py-1.5 px-4 rounded-xl cursor-pointer"
            >
              Retry
            </button>
          </div>
        ) : filteredStreaks.length === 0 ? (
          <div className="border border-slate-200 border-dashed rounded-xl p-12 text-center flex flex-col items-center gap-2">
            <span className="text-2xl">🔥</span>
            <h3 className="font-bold text-slate-700 text-sm">No active streaks</h3>
            <p className="text-slate-400 text-xs">
              Students need to check in consecutively to start building streaks!
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {filteredStreaks.map((entry) => (
              <div
                key={entry.student_id}
                className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col items-center text-center gap-3 shadow-sm"
              >
                <div className="w-12 h-12 rounded-full bg-slate-50 border border-slate-200 text-slate-600 font-bold text-base flex items-center justify-center">
                  {entry.display_name.charAt(0)}
                </div>
                <div className="flex flex-col">
                  <span className="font-bold text-slate-800 text-sm">{entry.display_name}</span>
                  <span className="text-slate-400 text-[10px] font-mono mt-0.5">ID: {entry.student_id}</span>
                </div>
                <div className="flex items-center gap-1.5 bg-orange-50 border border-orange-100 px-3 py-1 rounded-xl">
                  <span className="text-sm">🔥</span>
                  <span className="font-bold text-orange-700 text-xs">
                    {entry.current_streak} {entry.current_streak === 1 ? 'Day' : 'Days'} Streak
                  </span>
                </div>
                <div className="w-full flex flex-col gap-1.5 border-t border-slate-100 pt-3 mt-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider text-left pl-0.5">Badges</span>
                  {entry.badges.length > 0 ? (
                    <div className="flex flex-wrap gap-1.5 mt-1 justify-start">
                      {entry.badges.map((badge, bIdx) => (
                        <span
                          key={bIdx}
                          className={`border px-2 py-0.5 rounded-full text-[10px] font-bold ${getBadgeStyle(badge)}`}
                        >
                          {badge}
                        </span>
                      ))}
                    </div>
                  ) : (
                    <span className="text-[10px] text-slate-400 italic text-left pl-0.5">
                      No badges unlocked yet
                    </span>
                  )}
                </div>
                <div className="w-full text-slate-400 text-[9px] mt-1 border-t border-slate-50 pt-2 text-left flex justify-between">
                  <span>Last Active:</span>
                  <span className="font-semibold text-slate-500">{entry.last_check_in.split(' ')[0] || 'N/A'}</span>
                </div>
              </div>
            ))}
          </div>
        )}
        <div className="flex justify-end mt-2">
          <Link
            href="/"
            className="border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-sm py-2 px-5 rounded-xl transition-colors cursor-pointer text-center"
          >
            Return Home
          </Link>
        </div>
      </div>
    </div>
  );
}
